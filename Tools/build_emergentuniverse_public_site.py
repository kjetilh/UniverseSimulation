...
<body>
  <header>
    ...
    <p class="lead">A public, evidence-led archive testing whether local stochastic graph dynamics can produce robust higher-level law structure. The latest result is a repeatable finite event-poset interval contrast that survives a strict degree/depth/age control, while its effect magnitude is not stable enough across the frozen calibration transfer.</p>
  </header>
  <main>
    <section class="grid">
      ...
      <div class="card"><div class="metric">strict-null pass</div><p class="muted">The v16j local effect-existence gate passed in all 12 tested histories.</p></div>
      <div class="card"><div class="metric">0.4697</div><p class="muted">v16h/v16d effect-magnitude ratio; below the frozen stability bound of 0.5.</p></div>
    </section>
    ...
    <section>
      ...
      <p>The earlier defect/response track established nontrivial local interactions but not particles. The current architecture track records exact event dependencies. v16h retired a clock/depth common-geometry reading by validating scheduler total rate as the mechanism. v16i and v16j then found a separate open causal-interval abundance contrast.</p>
      <p>The frozen v16j composite gate failed only its effect-size transfer condition. The effect itself remained positive in 12/12 runs with empirical <code>p = 1/33</code>, and both growth and scheduler group checks passed. The required scientific reading is <code>strict_null_contrast_supported_magnitude_transfer_not_stable</code>.</p>
    ...
    <section>
      <h2>Current causal-structure gate</h2>
    ...
    <section>
      ...
      <p>v16j uses directed double-edge swaps that preserve event count, scheduler order, exact direct in/out-degree, exact causal depth and the global dyadic parent-age histogram. Every holdout null had to be unique and change at least 10% of direct edges.</p>
    </section>
    ...
    <section>
      <h2>Machine-readable interpretation audit</h2>
