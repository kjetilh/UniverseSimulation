#!/usr/bin/env python3
"""v0.15bz target-384 family probe.

After v15by showed that the target-192 plateau is broad but not cleanly
factorized, the next scale step is to test the same observable package at
target 384.
...
def run_seed_for(*, perturbation: str, placement: int, seed_delta: int) -> int:
    perturbation_offset = {"add_chord": 401, "local_swap": 457}[perturbation]
...
def diagnosis_rows(
    *,
    ...
    if largest_family >= 6 and repeated_non_mixed:
        status = "target384_family_plateau_supported"
        note = f"Target 384 viser en dominerende familie med {largest_family} av 8 profiler; {len(full_near)} full near-symmetry-kandidater."
        next_step = "holdout_target384_plateau"
        next_note = "Neste steg bor holde ut target-384 plateauet pa friske seeds."
    elif repeated_non_mixed and full_near:
        status = "target384_family_plus_symmetry_candidate"
        note = f"Target 384 har repeterte family-labels og {len(full_near)} full near-symmetry-kandidater."
        next_step = "holdout_target384_candidates"
        next_note = "Neste steg bor holde ut de konkrete target-384 kandidatene, ikke utvide søket."
    elif repeated_non_mixed:
        status = "target384_weak_family_signal"
        note = f"Target 384 har repeterte family-labels, men ikke et rent plateau; {len(support_only)} support-only near-symmetrier."
        next_step = "new_observable_or_next_scale"
        next_note = "Neste steg bor bytte observabel eller hoppe enda et hakk i skala."
    else:
        status = "target384_no_family_signal"
        note = "Target 384 gir ikke repeterte family-labels som rettferdiggjor videre lokal family-raffinering."
        next_step = "new_observable"
        next_note = "Neste steg bor vaere ny observabel, ikke mer family-label-tuning."
    ...
    return [
        ...
        {
            "diagnostic_family": "target384_family_probe",
            "status": status,
            "note": note,
        },
        {
...
def build_report(
    *,
    ...
    lines: List[str] = []
    lines.append("# Relasjonell universgraf v0.15bz: target-384 family probe")
    lines.append("")
    lines.append("## Formal")
    lines.append("")
    lines.append("Denne runden tar samme family-observabler ett hakk videre i skala til target `384`.")
    lines.append("Oppsettet er fortsatt smalt: samme growth_seed, samme placements, samme perturbasjoner.")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Target-384 profiler")
    lines.append("")
    ...
    lines.append("- Positivt signal her betyr bare at target 384 organiserer seg mer stabilt i samme feature-rom.")
    lines.append("- Symmetri skal ikke leses sterkere enn feature-avstandene tillater.")
    return "\n".join(lines) + "\n"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="v0.15bz target-384 family probe.")
    p.add_argument("--out-target-csv", type=str, default="Documentation/v15bz_target384_family_probe_target_summary.csv")
    p.add_argument("--out-runs-csv", type=str, default="Documentation/v15bz_target384_family_probe_rows.csv")
    p.add_argument("--out-aggregate-csv", type=str, default="Documentation/v15bz_target384_family_probe_aggregate.csv")
    p.add_argument("--out-family-csv", type=str, default="Documentation/v15bz_target384_family_probe_family_summary.csv")
    p.add_argument("--out-pairwise-csv", type=str, default="Documentation/v15bz_target384_family_probe_pairwise.csv")
    p.add_argument("--out-diagnosis-csv", type=str, default="Documentation/v15bz_target384_family_probe_diagnosis.csv")
    p.add_argument("--out-summary-md", type=str, default="Documentation/v15bz_target384_family_probe_lab.md")
    p.add_argument("--out-op-md", type=str, default="Documentation/v0_15bz_operativ_anbefaling.md")
    p.add_argument("--out-lay-md", type=str, default="Documentation/relasjonell_universgraf_for_ikke_spesialister_v0_15bz.md")
    return p.parse_args()
...
def main() -> None:
    ...
    op_md = "\n".join(
        [
            "# v0.15bz operativ anbefaling",
            "",
            *[f"- `{row['diagnostic_family']}`: `{row['status']}` fordi {row['note']}" for row in diagnosis],
            "",
            "- Les dette som et smalt target-384-probe, ikke som en bred ny scale scan.",
        ]
    ) + "\n"
    lay_md = "\n".join(
        [
            "# Relasjonell universgraf for ikke-spesialister v0.15bz",
            "",
            "Denne runden hopper enda et hakk opp i størrelse for å se om mønstrene blir tydeligere eller bare løser seg opp.",
            "",
            "Poenget er å teste om den samme målepakken finner mer orden ved 384 enn ved 192.",
        ]
    ) + "\n"
