#!/usr/bin/env python3
"""v0.13l refinement around the upper-band pivot candidate."""
...
LOWER_FINE = "bridge_00081640625_0000"
CENTER = "bridge_0008203125_0000"
UPPER_FINE = "bridge_00082421875_0000"
UPPER_ANCHOR = "bridge_000828125_0000"
...
def candidate_specs() -> List[Dict[str, Any]]:
    return [
        ...
        {
            "candidate": v09.ScaleCandidate(LOWER_FINE, 0.02, 0.00, 0.02, 0.00081640625, 0.00),
            "axis_group": "triad",
        },
        {
            "candidate": v09.ScaleCandidate(CENTER, 0.02, 0.00, 0.02, 0.0008203125, 0.00),
            "axis_group": "triad",
        },
        {
            "candidate": v09.ScaleCandidate(UPPER_FINE, 0.02, 0.00, 0.02, 0.00082421875, 0.00),
            "axis_group": "triad",
        },
        {
            "candidate": v09.ScaleCandidate(UPPER_ANCHOR, 0.02, 0.00, 0.02, 0.000828125, 0.00),
            "axis_group": "triad",
        },
    ]
...
def pivot_diagnosis(summary_rows: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if (
        ...
        pivot_status = "local_pivot_supported"
    elif (
...
def recommendation_rows(
    ...
    pivot_rows: Sequence[Dict[str, Any]],
    delta_rows: Sequence[Dict[str, Any]],
) -> List[Dict[str, Any]]:
    ...
    out.append(
        {
            ...
            "note": (
                "Den smale upper-pivot-runden bryter fortsatt `beta1` off-anchor; null-drift skal fortsatt ikke leses som lov."
                if beta1_breaks
                else "Null-driftene holder fortsatt eksakt i denne smale upper-pivot-runden."
            ),
        }
    )

    pivot = pivot_rows[0]
    status = str(pivot["pivot_status"])
    ...
    if status == "local_pivot_supported" and good_count >= 4:
        spectral_status = "good_but_local"
        spectral_note = "Punktet `bridge_0008203125_0000` holder som det reneste lokale pivotpunktet i upper-triad-familien, men sporet er fortsatt lokalt."
        validation_status = "yes_targeted"
        validation_note = "Et lite målrettet valideringssett rundt dette pivotpunktet er nå rimelig."
    elif status == "local_plateau" and mixed_count <= 1:
        spectral_status = "good_but_local"
        spectral_note = "Området rundt `bridge_0008203125_0000` ser mer ut som et lite lokalt plateau enn en skarp topp."
        validation_status = "yes_targeted"
        validation_note = "Et lite målrettet valideringssett kan brukes på dette smale pivot-plateauet."
    elif status == "pivot_not_supported":
        spectral_status = "mixed"
        spectral_note = "Det tilsynelatende pivotpunktet holder ikke bedre enn nabopunktene; spektralsporet er fortsatt blandet her."
        validation_status = "not_yet"
        validation_note = "Vent med bredere validering til pivot-området er bedre forstått."
    else:
        spectral_status = "mixed"
        spectral_note = "Pivot-området er fortsatt ikke rent nok til å kalle spektralsporet målrettet validert."
        validation_status = "not_yet"
        validation_note = "Vent med bredere validering til pivot-området er bedre avklart."
    ...
    out.append(
        {
            ...
            "best_candidate": "spectral_vs_dim_upper_pivot_refinement",
...
def build_report(
    ...
    lines: List[str] = []
    lines.append("# Relasjonell universgraf v0.13l: lokal raffinering av upper-pivot")
    lines.append("")
    lines.append("## Formål")
    lines.append("")
    lines.append(
    ...
    lines.append("")
    lines.append("## Pivot-diagnose")
    lines.append("")
    lines.append("| center | immediate_mean_p | p_gain_vs_immediate | margin_gain_vs_immediate | delta_improvement_vs_immediate | spectral_improvement_vs_immediate | pivot_status |")
    lines.append("| --- | --- | --- | --- | --- | --- | --- |")
    for row in pivot_rows:
        lines.append(
            ...
            f"{safe_float(row['delta_improvement_vs_immediate']):.4f} | {safe_float(row['spectral_improvement_vs_immediate']):.4f} | {row['pivot_status']} |"
        )
    lines.append("")
    ...
    lines.append("")
    lines.append("- Dette er ikke en ny scan. Det er en lokal pivot-test rundt `bridge_0008203125_0000`.")
    lines.append("- Hvis pivoten holder, vet vi at upper-bandet fra `v13j` og `v13k` egentlig samler seg rundt ett tydelig punkt.")
    lines.append("- Hvis den ikke holder, er området fortsatt lovende, men blandet.")
    lines.append("")
    return "\n".join(lines) + "\n"
...
def build_lay_summary(recommendation: Sequence[Dict[str, Any]]) -> str:
    ...
    lines = [
        "# v0.13l for ikke-spesialister",
        "",
        "Denne runden tester bare om ett lite punkt i den lovende øvre triad-sonen faktisk er bedre enn de nærmeste naboene sine.",
        "",
    ]
    ...
    lines.extend(["", "Poenget er å avgjøre om vi nå har ett tydelig midtpunkt, eller om også dette bare er del av et fortsatt blandet område.", ""])
    return "\n".join(lines)


def build_recommendation(recommendation: Sequence[Dict[str, Any]]) -> str:
    lines = ["# v0.13l operativ anbefaling", ""]
    for row in recommendation:
...
def build_status_note(pivot_rows: Sequence[Dict[str, Any]], recommendation: Sequence[Dict[str, Any]]) -> str:
    pivot = pivot_rows[0]
    spectral = next((r for r in recommendation if str(r["signal_family"]) == "spectral_quasi_invariant"), None)
    validation = next((r for r in recommendation if str(r["signal_family"]) == "larger_validation_set"), None)
    lines = [
        "# Relasjonell universgraf status v0.13l",
        "",
        "## Kort status",
        "",
        "- Dette er en lokal pivot-test etter `v13k`.",
        f"- Pivot-status rundt `{CENTER}`: `{pivot['pivot_status']}`.",
        f"- P-gain mot nærmeste naboer: `{safe_float(pivot['p_gain_vs_immediate']):.4f}`.",
        f"- Delta-forbedring mot nærmeste naboer: `{safe_float(pivot['delta_improvement_vs_immediate']):.4f}`.",
    ]
    if spectral is not None:
        lines.append(f"- Spektralsporet: `{spectral['status']}`.")
    if validation is not None:
    ...
    lines.append("- `v13l` sier om upper-bandet samler seg rundt ett reelt pivotpunkt eller fortsatt er en blandet lokal sone.")
...
def build_argparser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="v0.13l local upper pivot refinement")
    ...
    ap.add_argument("--output-prefix", default="Documentation/v13l")
    ap.add_argument("--report-md", default="Documentation/v13l_local_upper_pivot_refinement.md")
    ap.add_argument("--lay-md", default="Documentation/relasjonell_universgraf_for_ikke_spesialister_v0_13l.md")
    ap.add_argument("--recommendation-md", default="Documentation/v0_13l_operativ_anbefaling.md")
    ap.add_argument("--status-md", default="Documentation/relasjonell_universgraf_status_v0_13l.md")
    return ap
...
def main() -> None:
    ...
    print(
        f"[v13l] regime={regime.name} targets={targets} candidates={[c.name for c in candidates]} "
        f"growth={len(growth_seeds)} runs={len(run_offsets)} boot={args.bootstrap_reps}",
        flush=True,
    )
    print("[v13l] building bases...", flush=True)
    ...
    print("[v13l] bases done", flush=True)

    print("[v13l] collecting run rows...", flush=True)
    ...
    print(f"[v13l] runs done: {len(run_rows)} rows", flush=True)

    print("[v13l] aggregating candidate/base rows...", flush=True)
    ...
    pivot_rows = pivot_diagnosis(summary_rows)
    recommendation = recommendation_rows(summary_rows, pivot_rows, delta_rows)

    print("[v13l] writing outputs...", flush=True)
    ...
    write_csv(f"{prefix}_spectral_validation_pivot_diagnosis.csv", pivot_rows)
    write_csv(f"{prefix}_spectral_validation_recommendations.csv", recommendation)

    for path, content in [
        (args.report_md, build_report(target_summary, stable_rows, run_summary_rows, focus_summary_rows, pairwise_rows, delta_rows, summary_rows, pivot_rows, recommendation)),
        (args.lay_md, build_lay_summary(recommendation)),
        (args.recommendation_md, build_recommendation(recommendation)),
        (args.status_md, build_status_note(pivot_rows, recommendation)),
    ]:
        out = Path(path)
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_text(content, encoding="utf-8")
    print("[v13l] done", flush=True)
