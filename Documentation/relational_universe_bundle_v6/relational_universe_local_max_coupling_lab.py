...
def collect_multirun_statistics(args: argparse.Namespace, seeds: Sequence[int], coupling_modes: Sequence[str]) -> Dict[str, Any]:
    ...
    for mode, sub in by_mode.items():
        summaries[mode] = {
            ...
            "mean_final_radius_control": float(statistics.mean(float(r["final_radius_control"]) for r in sub)),
            "mean_total_unequal_time": float(statistics.mean(float(r["total_unequal_time"]) for r in sub)),
            "mean_avg_local_overlap": float(statistics.mean(float(r["avg_local_overlap_both_accept"]) for r in sub)),
            "mean_same_descriptor_rate": float(statistics.mean(float(r["avg_same_descriptor_both_accept"]) for r in sub)),
            "mean_shared_token_fraction_final": float(statistics.mean(float(r["shared_token_fraction_final"]) for r in sub)),
            "mean_shared_node_fraction_final": float(statistics.mean(float(r["shared_node_fraction_final"]) for r in sub)),
        }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--mode", type=str, default="single", choices=["single", "compare"])
...
def main() -> None:
    ...
    if args.mode == "single":
        ...
        with open(summary_path, "w", encoding="utf-8") as f:
            f.write(make_single_summary_md(args, res, log_path, ev_path))
    ...
    if args.mode == "compare":
        ...
        with open(summary_path, "w", encoding="utf-8") as f:
            f.write(make_multirun_summary_md(args, stats, csv_path))
        with open(status_path, "w", encoding="utf-8") as f:
            f.write(make_status_md(args, stats))
        with open(overview_path, "w", encoding="utf-8") as f:
            f.write(make_project_overview_md(args, stats))
        with open(lay_path, "w", encoding="utf-8") as f:
            f.write(make_lay_md(stats))
        with open(readme_path, "w", encoding="utf-8") as f:
            f.write(make_readme_md())
        print(json.dumps({
