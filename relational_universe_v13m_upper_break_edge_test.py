#!/usr/bin/env python3
"""v0.13m test of the upper break edge around bridge_00082421875_0000."""
...
LOWER_FINE = "bridge_000822265625_0000"
CENTER = "bridge_00082421875_0000"
UPPER_FINE = "bridge_000826171875_0000"
...
def candidate_specs() -> List[Dict[str, Any]]:
    return [
        ...
        {
            "candidate": v09.ScaleCandidate(LOWER_FINE, 0.02, 0.00, 0.02, 0.000822265625, 0.00),
            "axis_group": "triad",
        },
        {
            "candidate": v09.ScaleCandidate(CENTER, 0.02, 0.00, 0.02, 0.00082421875, 0.00),
            "axis_group": "triad",
        },
        {
            "candidate": v09.ScaleCandidate(UPPER_FINE, 0.02, 0.00, 0.02, 0.000826171875, 0.00),
            "axis_group": "triad",
        },
        {
...
def recommendation_rows(
    ...
    out.append(
        {
            ...
            "note": (
                "Den smale upper-break-runden bryter fortsatt `beta1` off-anchor; null-drift skal fortsatt ikke leses som lov."
                if beta1_breaks
                else "Null-driftene holder fortsatt eksakt i denne smale upper-break-runden."
            ),
        }
    )
    ...
    if status == "local_break_supported" and sharp_count >= 3:
        ...
        spectral_note = "Punktet `bridge_00082421875_0000` ser ut til å markere en ekte lokal bruddkant i upper-triad-familien, men sporet er fortsatt lokalt."
        validation_status = "yes_targeted"
        validation_note = "Et lite målrettet valideringssett rundt denne bruddkanten er nå rimelig."
    elif status == "local_plateau" and mixed_count <= 1:
        spectral_status = "good_but_local"
        spectral_note = "Området rundt `bridge_00082421875_0000` ser mer ut som et lite overgangsplateau enn en skarp bruddkant."
        validation_status = "yes_targeted"
        ...
        spectral_status = "good_but_local"
        spectral_note = "Den tilsynelatende bruddkanten holder ikke som egen lokal knekk; området ser renere ut enn `v13l` alene tilsa."
        validation_status = "yes_targeted"
        validation_note = "Et lite målrettet valideringssett rundt den øvre fine delen er nå rimelig."
    else:
        spectral_status = "mixed"
        spectral_note = "Bruddkanten er fortsatt ikke ren nok til å kalle spektralsporet målrettet validert."
        validation_status = "not_yet"
        validation_note = "Vent med bredere validering til den øvre bruddkanten er bedre avklart."
    ...
    out.append(
        {
            ...
            "best_candidate": "spectral_vs_dim_upper_break_edge",
...
def build_report(
    ...
    lines: List[str] = []
    lines.append("# Relasjonell universgraf v0.13m: test av øvre bruddkant")
    lines.append("")
    lines.append("## Formål")
    lines.append("")
    lines.append(
    ...
    lines.append("")
    lines.append("## Bruddkant-sammendrag")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Bruddkant-diagnose")
    lines.append("")
    ...
    lines.append("")
    lines.append("- Dette er ikke en ny scan. Det er en smal test av den øvre bruddkanten rundt `bridge_00082421875_0000`.")
    lines.append("- Hvis bruddkanten holder, vet vi at usikkerheten i `v13l` faktisk sitter i en lokal overgang og ikke i hele området.")
    lines.append("- Hvis den ikke holder, er den øvre delen renere enn `v13l` alene tilsa.")
    lines.append("")
    return "\n".join(lines) + "\n"
...
def build_lay_summary(recommendation: Sequence[Dict[str, Any]]) -> str:
    ...
    lines = [
        "# v0.13m for ikke-spesialister",
        "",
        "Denne runden tester bare om det svake punktet i den øvre triad-sonen faktisk er en ekte lokal bruddkant, eller om området er jevnere enn det så ut til før.",
        "",
    ]
...
def build_recommendation(recommendation: Sequence[Dict[str, Any]]) -> str:
    lines = ["# v0.13m operativ anbefaling", ""]
    for row in recommendation:
...
def build_status_note(break_rows: Sequence[Dict[str, Any]], recommendation: Sequence[Dict[str, Any]]) -> str:
    ...
    validation = next((r for r in recommendation if str(r["signal_family"]) == "larger_validation_set"), None)
    lines = [
        "# Relasjonell universgraf status v0.13m",
        "",
        "## Kort status",
        "",
        "- Dette er en smal test av den øvre bruddkanten etter `v13l`.",
    ...
    lines.append("- `v13m` sier om `bridge_00082421875_0000` er en reell lokal knekk eller bare en midlertidig svak måling.")
...
def build_argparser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="v0.13m upper break edge test")
    ...
    ap.add_argument("--output-prefix", default="Documentation/v13m")
    ap.add_argument("--report-md", default="Documentation/v13m_upper_break_edge_test.md")
    ap.add_argument("--lay-md", default="Documentation/relasjonell_universgraf_for_ikke_spesialister_v0_13m.md")
    ap.add_argument("--recommendation-md", default="Documentation/v0_13m_operativ_anbefaling.md")
    ap.add_argument("--status-md", default="Documentation/relasjonell_universgraf_status_v0_13m.md")
    return ap
...
def main() -> None:
    ...
    print(
        f"[v13m] regime={regime.name} targets={targets} candidates={[c.name for c in candidates]} "
        f"growth={len(growth_seeds)} runs={len(run_offsets)} boot={args.bootstrap_reps}",
        flush=True,
    )
    print("[v13m] building bases...", flush=True)
    ...
    print("[v13m] bases done", flush=True)

    print("[v13m] collecting run rows...", flush=True)
    ...
    print(f"[v13m] runs done: {len(run_rows)} rows", flush=True)

    print("[v13m] aggregating candidate/base rows...", flush=True)
    ...
    print("[v13m] writing outputs...", flush=True)
